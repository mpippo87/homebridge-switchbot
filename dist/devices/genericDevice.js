...
export class WaterDetectorDevice extends GenericDevice {
    ...
    lastBatteryLevel;
    _batteryRefreshing = false;
    _batteryRefreshTs = 0;
    BATTERY_REFRESH_TTL_MS = 30_000;
    async init() {
        await super.init();
    ...
    async getOpenAPIStatus() {
        ...
        try {
            ...
            this.log?.debug?.(`[WaterDetector] direct OpenAPI refresh failed: ${e?.message}`);
        }
        return undefined;
    }
    ...
    async _refreshBattery() {
        ...
        try {
            ...
            this.log?.debug?.(`[WaterDetector] battery refresh failed: ${e?.message}`);
        }
        finally {
